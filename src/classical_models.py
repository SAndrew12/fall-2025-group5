...
class ModelTrainer:
    def __init__(self, X_train, y_train, X_test, y_test, random_seed=42):
    ...
    def get_models(self):
        return {
            'mlpc': MLPClassifier(early_stopping=True, random_state=self.random_seed),
            'rfc': RandomForestClassifier(class_weight='balanced', random_state=self.random_seed),
            'xgb': XGBClassifier(random_state=self.random_seed, eval_metric='logloss')
        }
